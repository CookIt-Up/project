...
class RecipeServicePage extends StatelessWidget {
...
Future<void> deleteDocumentAndSubcollections(String documentId) async {
  ...
  void _approveRecipe(String recipeId) {
    final CollectionReference recipesCollection =
        FirebaseFirestore.instance.collection('recipe');

    // Update 'approved' field of the recipe document to true
    recipesCollection
        .doc(recipeId)
        .update({'approved': true})
        .then((value) {
      print('Recipe approved successfully');
      // Optionally, trigger a UI update
    }).catchError((error) {
      print('Failed to approve recipe: $error');
